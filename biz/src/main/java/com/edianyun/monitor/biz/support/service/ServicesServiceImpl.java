package com.edianyun.monitor.biz.support.service;
...
import com.edianyun.monitor.bean.bizBean.HostBO;
import com.edianyun.monitor.bean.bizBean.ServiceBO;
import com.edianyun.monitor.biz.dubboService.DubboMonitorService;
...
public class ServicesServiceImpl implements ServicesService {
    ...
    public Map<String, Set<String>> getServiceProviders() {
        return getService(Constants.PROVIDERS_CATEGORY);
    }

    @Override
    public Map<String, Set<String>> getServiceConsumers() {
        return getService(Constants.CONSUMERS_CATEGORY);
    }

    private Map<String, Set<String>> getService(String category){
        Map<String, Set<String>> serviceMap = new ConcurrentHashMap<>();
        Map<String, Map<String, Set<URL>>> registry = registryContainer.getRegistryCache();
        Map<String, Set<URL>> consumeServices = registry.get(category);

        for (Map.Entry<String, Set<URL>> serviceEntry : consumeServices.entrySet()) {
            String service = serviceEntry.getKey();
            Set<String> applications = serviceMap.get(service);
            if(null == applications){
                applications = new ConcurrentHashSet<>();
                serviceMap.put(service,applications);
            }
    ...
    private void ServiceBOSetMethods(ServiceBO serviceBO,URL url){
        ...
        if(null == methodSet){
            ...
            List<String> newList = Arrays.asList(methods.split(","));
            Boolean isWrong = false;
            if(oldList.size() != newList.size()) {
                isWrong = true;
            }
            for(String oneMethod : newList){
