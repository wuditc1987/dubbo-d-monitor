package com.edianyun.monitor.biz.dubboService;
...
import com.edianyun.monitor.bean.bizBean.ApplicationChangeBO;
import com.edianyun.monitor.biz.support.service.AppChangeService;
import com.edianyun.monitor.common.tools.SpringContextsUtil;
import com.edianyun.monitor.common.tools.Tool;
...
public class RegistryContainerImpl implements RegistryContainer {
    ...
    public Map<String, Map<String, Set<URL>>> getRegistryCache() {
        if (!registryCache.containsKey(Constants.PROVIDERS_CATEGORY)) {
            registryCache.put(Constants.PROVIDERS_CATEGORY, new ConcurrentHashMap<>());
        }
        if (!registryCache.containsKey(Constants.CONSUMERS_CATEGORY)) {
            registryCache.put(Constants.CONSUMERS_CATEGORY, new ConcurrentHashMap<>());
        }
        if (!registryCache.containsKey(Constants.CONFIGURATORS_CATEGORY)) {
            registryCache.put(Constants.CONFIGURATORS_CATEGORY, new ConcurrentHashMap<>());
        }
    ...
    private void removeFromChangeAppCaChe(URL url) {
        ...
        ApplicationChangeBO applicationChangeBO = getApplicationChangeBO(url);
    ...
    private ApplicationChangeBO getApplicationChangeBO(URL url){
        ...
        return new ApplicationChangeBO(host, port, application, category, organization);
    }
